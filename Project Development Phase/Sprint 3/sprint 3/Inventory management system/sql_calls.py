    ...
    def sale(admin_email,customer_name,customer_email,phone_number,product_id,quantity,user_id):
        ...
        if(quantity<=stock):
            ...
            if existing_profit == None:
                existing_profit = 0
            profit = ((existing_profit)+((selling_price*quantity)-(purchase_price*quantity)))
...
class create_table:
    def item_table(email):
        user_id=check_query_data.check_signle_column('user','email',email)
        sql=f"CREATE TABLE product_details_{user_id[0]}(product_id BIGINT NOT NULL GENERATED ALWAYS AS IDENTITY (START WITH 1, INCREMENT BY 1),supplier VARCHAR(30),date timestamp DEFAULT CURRENT_TIMESTAMP,product_name VARCHAR(30) NOT NULL UNIQUE,purchase_price BIGINT,selling_price BIGINT,stock BIGINT,total_selling_price BIGINT,status VARCHAR(10),low_stock BIGINT,PRIMARY KEY (product_id));"
    ...
    def sales_table(admin_email):
        ...
        sql = f"CREATE TABLE sales_details_{user_id[0]}(sales_id BIGINT NOT NULL GENERATED ALWAYS AS IDENTITY (START WITH 1, INCREMENT BY 1),customer_name VARCHAR(30),customer_email VARCHAR(30),product_id BIGINT,phone_number BIGINT,date timestamp DEFAULT CURRENT_TIMESTAMP,product_name VARCHAR(30),quantity BIGINT,billing_amount BIGINT,PRIMARY KEY (sales_id));"
        ibm_db.exec_immediate(con, sql)
...
class send_mail:
    ...
    def mail(email):
        sg = sendgrid.SendGridAPIClient(api_key='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')
        ...
        html_content=f'http://127.0.0.1:5000/forgot_password_verify/{encoded_email}'
